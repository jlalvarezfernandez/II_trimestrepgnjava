/**
 * Controlador dedicado a pedir un código de un artículo para incrementar el número de unidades 
 * de un artículo del almacén
 * 
 * @author JOSÉ LUIS ÁLVAREZ FÉRNANDEZ
 * @author FRANCISCO JAVIER GUTIERREZ CAMPOS
...
public class CodigoEntradaMercanciaController implements Initializable{
  ...
  /**
   * Método apra saber si existe un código de un artículo del alamcén
   * @param codigo
   * @return
  ...
  /**
   * Método aceptar para que al introducir un código correcto nos lleve a la ventana correspondiente para incrementar el número de unidades de un artículo
   * @param event
   * @throws IOException
   */
  ...
  public void aceptar(ActionEvent event) throws IOException {
    ...
    try {
      ...
      if (existe(codigo)) {
        FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/application/vistas/EntradaMercancia.fxml"));
        root = fxmlLoader.load();

        EntradaMercanciaController controller = fxmlLoader.getController();
        controller.set(codigo);

        Scene escena = new Scene(root);
        Stage escenario = new Stage();
        escenario.setTitle("Sección dedicada a incrementar las unidades de un artículo");
